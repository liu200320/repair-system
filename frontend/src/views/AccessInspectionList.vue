...
import { listAccessInspections, deleteAccessInspection, exportAccessInspectionRange } from '../api/accessInspection'
...
async function fetchList() {
  ...
  try {
    ...
    const res = await listAccessInspections(params)
...
async function handleDelete(row) {
  ...
  await deleteAccessInspection(row.id)
...
async function handleExport(row) {
  ...
  try {
    ...
    const resp = await fetch(`/api/v1/access-inspections/${row.id}/export`, {
    ...
    a.href = url; a.download = `门禁日常巡检表_${row.record_no}.docx`
...
async function handleExportRange() {
  if (!exportRange.value || exportRange.value.length < 2) {
    ElMessage.warning('请先选择日期范围'); return
  }
  const [start, end] = exportRange.value
  exportingRange.value = true
  try {
    await exportAccessInspectionRange(start, end)
    ElMessage.success('批量导出成功，文件已下载')
    rangeDialogVisible.value = false; exportRange.value = []
  } catch (err) {
  ...
  <div>
      ...
      <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:8px;">
        <el-form inline style="flex:1;">
          <el-form-item label="门禁地点">
            <el-input v-model="query.location" placeholder="模糊搜索" clearable style="width:200px" @keyup.enter="handleSearch" />
          </el-form-item>
        ...
        <div style="display:flex; gap:8px; flex-shrink:0;">
          <el-button type="warning" @click="rangeDialogVisible = true">按时间段批量导出 Word</el-button>
          <el-button type="success" @click="router.push('/access-inspections/create')">
    ...
    <el-card shadow="never">
      <el-table :data="records" v-loading="loading" stripe border style="width:100%">
        ...
        <el-table-column prop="location"     label="门禁地点" min-width="200" show-overflow-tooltip />
        <el-table-column prop="inspector"    label="巡检人员" width="120" />
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            <el-button size="small" @click="router.push(`/access-inspections/${row.id}`)">详情</el-button>
            <el-button size="small" type="primary" @click="router.push(`/access-inspections/${row.id}/edit`)">编辑</el-button>
      ...
      <div style="display:flex; justify-content:flex-end; margin-top:16px;">
        <el-pagination v-model:current-page="query.page" v-model:page-size="query.page_size"
          ...
          @current-change="fetchList" @size-change="() => { query.page = 1; fetchList() }" />
      </div>
    </el-card>

    <el-dialog v-model="rangeDialogVisible" title="按时间段批量导出 Word" width="420px" :close-on-click-modal="false">
      <div style="padding:8px 0;">
        <p style="margin-bottom:16px; color:#606266;">
          选择日期范围后，该时间段内所有门禁巡检记录将汇总到<strong>一个 Word 文档</strong>，每条记录独占一页，含照片。
        </p>
        <el-form label-width="80px">
          <el-form-item label="时间范围">
            <el-date-picker v-model="exportRange" type="daterange" value-format="YYYY-MM-DD"
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="exportingRange"
          :disabled="!exportRange || exportRange.length < 2" @click="handleExportRange">
