...
from app.models.access_inspection import AccessInspectionRecord
...
def _hdr(cell, text, bg="D4EDDA", align=WD_ALIGN_PARAGRAPH.CENTER):
    cell.text = text
...
def _write_single_record(doc, record: AccessInspectionRecord):
    # 标题
    tp = doc.add_paragraph(); _zero(tp); tp.alignment = WD_ALIGN_PARAGRAPH.CENTER
    tr = tp.add_run("云南农业职业技术学院门禁日常巡检登记表")
    ...
    _hdr(info.rows[1].cells[0], "门禁地点"); _val(info.rows[1].cells[1], record.location)
    ...
    _hdr(st.rows[0].cells[0], "道闸情况"); _hdr(st.rows[0].cells[1], "翼闸情况")
    _hdr(st.rows[0].cells[2], "门禁系统情况"); _hdr(st.rows[0].cells[3], "其他设备")
    _val(st.rows[1].cells[0], record.gate_status or "")
    _val(st.rows[1].cells[1], record.flap_status or "")
    _val(st.rows[1].cells[2], record.system_status or "")
    _val(st.rows[1].cells[3], record.other_device or "")

    # 故障描述 + 维修内容
    dt = doc.add_table(rows=2, cols=2); dt.style = "Table Grid"
    for row in dt.rows:
        row.cells[0].width = Cm(2.4); row.cells[1].width = Cm(14.2)
...
def export_access_inspection_to_word(record: AccessInspectionRecord, output_path: str) -> str:
...
def export_access_inspection_range_to_word(records, start_date, end_date, output_path):
    doc = Document(); _set_doc_margins(doc)
